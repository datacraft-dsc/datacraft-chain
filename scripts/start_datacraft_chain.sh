...
# start_datacraft_chain.sh


function show_help {
cat  << EOT

start_datacraft_chain.sh  [options] <command>

Start the datacraft chain or local private network.
...
test                    Start a local private network and also install the contracts on the local network.
                        The same as 'start_datacraft_chain.sh --deploy local'.

Options:

--docker=<DOCKER_NAME>                  Docker image name. Default: 'datacraft-chain'.
...
DOCKER_NAME="datacraft-chain"
DOCKER_REPO="docker.pkg.github.com/datacraft-dsc/datacraft-chain"
